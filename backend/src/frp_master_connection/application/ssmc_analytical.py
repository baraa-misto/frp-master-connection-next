...
from frp_master_connection.application.ssmc_cuts import SSMCCutLedger, build_ssmc_cut_ledger
...
def _actions(preview: SSMCPreview) -> tuple[SSMCActionReaction, ...]:
    ...
    for index, (group, member) in enumerate(
        ...
        planar = (
            native is not None
            and native.status == "CALCULATED"
            and native.solution.proof is not None
            and native.solution.proof.passed
        )
        records.append(
            SSMCActionReaction(
                group.group_id,
...
def evaluate_ssmc_analytical(
    ...
    for group in preview.groups:
        native = cast(InPlaneWrenchResult | None, group.native_slice8)
        if native is not None and native.status == "CALCULATED":
            group_bolts[group.group_id] = {bolt.bolt_id: bolt for bolt in native.solution.bolts}
