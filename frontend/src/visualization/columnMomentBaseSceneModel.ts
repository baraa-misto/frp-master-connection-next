...
import type { ColumnMomentBasePreview } from "../api/columnMomentBaseContracts";
...
export function buildColumnMomentBaseScene(p:ColumnMomentBasePreview):SingleBoltSceneModel {
  const boxes:SceneBox[]=p.geometry.display_parts.map(v=>({id:v.part_id,label:label(v.part_id),ownerId:v.box.component_id,ownerLabel:label(v.box.component_id),ownerRole:v.box.component_id==="FOUNDATION"?"OTHER":v.box.component_id==="COLUMN"?"COLUMN":"BRACE",elementId:v.part_id,materialRegionId:v.material_region?.region_id??null,
  ...
  const cylinders:SceneCylinder[]=p.geometry.member_bolts.flatMap(b=>{
    ...
    return [
      ...
      {...common,id:`${b.hardware_id}:HOLE`,label:"Physical continuous member-hole path",start:point(b.start),end:point(b.end),diameter:inches(b.hole_diameter),kind:"HOLE"},
  ...
  const materialAxes:SceneMaterialAxes[]=p.geometry.display_parts.flatMap(v=>{
    ...
    return [{id:`${v.part_id}:MATERIAL`,...source,componentLabel:label(r.component_id),sectionFamily:r.component_id==="COLUMN"?(p.input.column.family==="WI"?"WIDE_FLANGE_I":p.input.column.family==="RHS"?"RECTANGULAR_HOLLOW_SECTION":"SOLID_RECTANGULAR_SECTION"):"ANGLE",elementLabel:label(r.region_id),origin:presentation?.origin??{x:inches(v.box.center_l_v_t.l),y:inches(v.box.center_l_v_t.v),z:inches(v.box.center_l_v_t.t)},presentation}];
  });
  const origin=point(p.column_on_base.reference);
  const axes:readonly (readonly ["FX"|"FY"|"FZ"|"MX"|"MY",MultiRowQuantity,Vec3])[]=[
    ["FX",p.column_on_base.force.x,BASIS[0]],["FY",p.column_on_base.force.y,BASIS[1]],["FZ",p.column_on_base.force.z,BASIS[2]],["MX",p.column_on_base.moment.x,BASIS[0]],["MY",p.column_on_base.moment.y,BASIS[1]],
  ];
