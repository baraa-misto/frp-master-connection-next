import type { WIMomentSpliceVector, WIMomentSpliceVisualization } from "../api/wiMomentSpliceContracts";
...
const point = (value: WIMomentSpliceVector): Vec3 => ({ x: q(value.l), y: q(value.t), z: q(value.v) });
const axis = (value: readonly [string, string, string]): Vec3 => ({ x: Number(value[0]), y: Number(value[2]), z: Number(value[1]) });

function boxes(value: WIMomentSpliceVisualization): readonly SceneBox[] {
  return value.boxes.map((box) => {
    ...
    return { id: box.component_id, label: box.role.replaceAll("_", " "), ownerId, ownerLabel: ownerId.replaceAll("_", " "), ownerRole: "BRACE" as const, elementId: box.component_id, materialRegionId: box.component_id, center: point(box.center_l_v_t), size: { x: q(box.size_l_v_t.l), y: q(box.size_l_v_t.t), z: q(box.size_l_v_t.v) }, basis: [{ x: 1, y: 0, z: 0 }, { x: 0, y: 1, z: 0 }, { x: 0, y: 0, z: 1 }], deferred: false, interference: false };
  });
}

function cylinders(value: WIMomentSpliceVisualization): readonly SceneCylinder[] {
...
export function buildWIMomentSpliceSceneModel(value: WIMomentSpliceVisualization): SingleBoltSceneModel {
  ...
  const materialAxes: SceneMaterialAxes[] = value.material_regions.map((region) => {
    ...
    return { id: `wi-moment-splice:${region.region_id}`, componentId: region.component_id, componentLabel: box?.ownerLabel ?? region.component_id, sectionFamily: region.component_id.startsWith("BEAM") ? "WIDE_FLANGE" : "PLATE", elementId: region.region_id, elementLabel: region.region_id.replaceAll("_", " "), materialRegionId: region.region_id, origin: presentation?.origin ?? box?.center ?? { x: 0, y: 0, z: 0 }, presentation, lengthwise: source.lengthwise, crosswise: source.crosswise, throughThickness: source.throughThickness };
  });
  const origin = point(value.action_reference_l_v_t);
  const linear = (component: "FX" | "FY" | "FZ", raw: number, positive: Vec3): SceneArrow | null => raw === 0 ? null : { id: `wi-moment-splice-${component}`, component, kind: "LINEAR", origin, axis: raw < 0 ? { x: -positive.x, y: -positive.y, z: -positive.z } : positive, signedValue: raw, unit: value.applied_force_l_v_t.l.unit, sense: raw < 0 ? "NEGATIVE" : "POSITIVE", isZero: false, referencePointId: "WI_MOMENT_SPLICE_JOINT", frameId: "WI_MOMENT_SPLICE_FRAME", axialLoadingSense: component === "FX" ? raw < 0 ? "COMPRESSION" : "TENSION" : null };
  const moment = q(value.applied_moment_l_v_t.t);
  const momentArrow: SceneArrow | null = moment === 0 ? null : { id: "wi-moment-splice-MY", component: "MY", kind: "ROTATIONAL", origin, axis: moment < 0 ? { x: 0, y: -1, z: 0 } : { x: 0, y: 1, z: 0 }, signedValue: moment, unit: value.applied_moment_l_v_t.t.unit, sense: moment < 0 ? "NEGATIVE" : "POSITIVE", isZero: false, referencePointId: "WI_MOMENT_SPLICE_JOINT", frameId: "WI_MOMENT_SPLICE_FRAME", axialLoadingSense: null };
  const force = value.applied_force_l_v_t;
  const appliedArrows = [linear("FX", q(force.l), { x: 1, y: 0, z: 0 }), linear("FZ", q(force.v), { x: 0, y: 0, z: 1 }), linear("FY", q(force.t), { x: 0, y: 1, z: 0 }), momentArrow].filter((item): item is SceneArrow => item !== null);
  const bounds = calculatePresentationBounds([...sceneBoxes.flatMap(boxPoints), ...sceneCylinders.flatMap((item) => [item.start, item.end])]);
  return { snapshotVersion: "4.1A-PREVIEW-RC1", unitSystem: value.beam_end_planes_l[0].unit === "in" ? "US_CUSTOMARY" : "SI", lengthUnit: value.beam_end_planes_l[0].unit, boxes: sceneBoxes, meshes: [], cylinders: sceneCylinders, frames: [{ id: "WI_MOMENT_SPLICE_FRAME", label: "W/I moment-splice L_S / T_S / V_S frame", kind: "JOINT_LOCAL", ownerId: null, origin, xAxis: { x: 1, y: 0, z: 0 }, yAxis: { x: 0, y: 1, z: 0 }, zAxis: { x: 0, y: 0, z: 1 }, valid: true }], markers: [{ id: "WI_MOMENT_SPLICE_JOINT", label: "Joint transfer reference", kind: "REFERENCE_POINT", position: origin, connected: null, memberEnd: null }], positiveArrows: [], appliedArrows, connectionDemandArrows: [], perBoltDemandArrows: [], materialAxes, zones: [], connectionOrientation: null, boundsCenter: bounds.center, boundsRadius: bounds.radius, fitCenter: bounds.center, fitRadius: bounds.radius * 1.08 };
}
