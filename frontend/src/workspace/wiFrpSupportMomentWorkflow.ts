...
import type { WIFrpSupportMomentPreview, WIFrpSupportMomentRequest, WIFrpSupportMomentResponse } from "../api/wiFrpSupportMomentContracts";
import { previewWIFrpSupportMoment } from "../api/wiFrpSupportMomentClient";
import { isIntentionalAbort, PREVIEW_DEBOUNCE_MS } from "./previewWorkflow";

export function useWIFrpSupportMomentPreview(request: WIFrpSupportMomentRequest, revision: number, immediate: boolean, invalid: string | null) {
  const [response,setResponse] = useState<WIFrpSupportMomentResponse<WIFrpSupportMomentPreview> | null>(null);
  ...
  useEffect(() => {
    ...
    const execute = async () => {
      try {
        const next = await previewWIFrpSupportMoment(request,controller.signal);
