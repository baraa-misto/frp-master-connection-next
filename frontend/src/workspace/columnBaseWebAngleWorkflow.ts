...
import type { ColumnBasePreviewResponse, ColumnBaseWebAngleRequest } from "../api/columnBaseWebAngleContracts";
import { EvaluationTransportError, previewColumnBaseWebAngle } from "../api/client";
import { isIntentionalAbort, PREVIEW_DEBOUNCE_MS } from "./previewWorkflow";

export type ColumnBasePreviewState =
  | "CURRENT_VALID"
  | "PREVIEW_PENDING"
...
export function useColumnBaseWebAnglePreview(input: {
  readonly request: ColumnBaseWebAngleRequest;
  readonly revision: number;
  readonly immediate: boolean;
  readonly validationMessage: string | null;
}) {
  const { request, revision, immediate, validationMessage } = input;
  const [response, setResponse] = useState<ColumnBasePreviewResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const execute = async () => {
      setFailure(null);
      try {
        const result = await previewColumnBaseWebAngle(request, controller.signal);
        ...
        const error = caught instanceof EvaluationTransportError
          ? caught
          : new EvaluationTransportError("RESPONSE", null, "Unexpected column-base preview failure.", caught);
  ...
  let state: ColumnBasePreviewState;
