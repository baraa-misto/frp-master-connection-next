...
import { EvaluationTransportError, previewDirectSideLapConcrete } from "../api/client";
import { isIntentionalAbort, PREVIEW_DEBOUNCE_MS } from "./previewWorkflow";

export type DirectSideLapPreviewState = "CURRENT_VALID" | "PREVIEW_PENDING" | "CURRENT_INVALID_SHOWING_LAST_VALID" | "PREVIEW_FAILED_SHOWING_LAST_VALID" | "NO_VALID_PREVIEW";

export function useDirectSideLapConcretePreview(input: {
  readonly request: DirectSideLapConcreteRequest;
  readonly revision: number;
  readonly immediate: boolean;
  readonly validationMessage: string | null;
}) {
  const { request, revision, immediate, validationMessage } = input;
  const [response, setResponse] = useState<DirectSideLapPreviewResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const execute = async () => {
      setFailure(null);
      try {
        const result = await previewDirectSideLapConcrete(request, controller.signal);
        ...
        const error = caught instanceof EvaluationTransportError ? caught : new EvaluationTransportError("RESPONSE", null, "Unexpected direct side-lap preview failure.", caught);
  ...
  const state: DirectSideLapPreviewState = validationMessage !== null
    ? hasLastValid ? "CURRENT_INVALID_SHOWING_LAST_VALID" : "NO_VALID_PREVIEW"
    : acceptedRevision === revision ? "CURRENT_VALID"
      : currentFailure?.kind === "INVALID" ? hasLastValid ? "CURRENT_INVALID_SHOWING_LAST_VALID" : "NO_VALID_PREVIEW"
        : currentFailure?.kind === "ERROR" ? hasLastValid ? "PREVIEW_FAILED_SHOWING_LAST_VALID" : "NO_VALID_PREVIEW"
          : "PREVIEW_PENDING";
