...
export function BeamConcretePairedAngleWorkspace() {
  ...
  return <ConnectionWorkspaceShell family="beam-concrete-paired-angle" className="clip-angle-workspace paired-clip-angle-workspace beam-concrete-workspace" banner={<section className="workspace-banner"><div><p className="eyebrow">Stage 3.5A-R2 · Shear-category external-anchor handoff</p><h2>Beam connection — Paired clip angles to concrete wall</h2><p>Backend-authoritative FRP connection design with concrete and anchor capacity designed elsewhere.</p></div><div className="benchmark-actions"><button type="button" className="secondary-button" onClick={() => { loadBenchmark("US_CUSTOMARY"); }}>Load R2 U.S.</button><button type="button" className="secondary-button" onClick={() => { loadBenchmark("SI"); }}>Load R2 SI</button></div></section>}>
