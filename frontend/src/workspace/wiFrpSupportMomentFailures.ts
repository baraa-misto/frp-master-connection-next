...
import type { WIFrpSupportMomentDesign } from "../api/wiFrpSupportMomentContracts";
...
export function frpSupportMomentVisibleFailures(value: WIFrpSupportMomentDesign): readonly WallMomentVisibleFailure[] {
  ...
  for (const c of value.beam_bearings) {
  ...
  for (const c of value.beam_local_checks) {
  ...
  for (const c of value.support_bolts) {
    if (c.status === "FAIL") rows.push({checkId:c.check_id,component:c.connector_id,reason:c.reason,utilization:null,demand:c.total_tension_including_prying,resistance:null,nativeRecord:c});
  }
  for (const c of value.support_local_checks) {
    if (c.status === "FAIL") rows.push({checkId:c.check_id,component:c.layer_id,reason:c.method,utilization:null,demand:c.demand,resistance:c.resistance,nativeRecord:c});
  }
  if (value.local_zone?.status === "FAIL") rows.push({checkId:"QUALIFIED_LOCAL_SUPPORT_ZONE",component:"FRP support assembly",reason:value.local_zone.reasons.join("; "),utilization:null,demand:null,resistance:null,nativeRecord:value.local_zone});
