...
import type { WebSplicePreviewResponse, WebSpliceRequest } from "../api/webSpliceContracts";
import { EvaluationTransportError, previewWebSplice } from "../api/client";
import { isIntentionalAbort, PREVIEW_DEBOUNCE_MS } from "./previewWorkflow";

export function useWebSplicePreview(input: { readonly request: WebSpliceRequest; readonly revision: number; readonly immediate: boolean; readonly validationMessage: string | null }) {
  const { request, revision, immediate, validationMessage } = input;
  const [response, setResponse] = useState<WebSplicePreviewResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const execute = async () => {
      setFailure(null);
      try {
        const result = await previewWebSplice(request, controller.signal);
        ...
        const error = caught instanceof EvaluationTransportError ? caught : new EvaluationTransportError("RESPONSE", null, "Unexpected web-splice preview failure.", caught);
