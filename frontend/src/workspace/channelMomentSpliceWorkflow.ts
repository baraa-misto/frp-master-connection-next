...
import { EvaluationTransportError, previewChannelMomentSplice } from "../api/client";
import type { ChannelMomentSplicePreviewResponse, ChannelMomentSpliceRequest } from "../api/channelMomentSpliceContracts";
import { isIntentionalAbort, PREVIEW_DEBOUNCE_MS } from "./previewWorkflow";

export function useChannelMomentSplicePreview(input: {
  readonly request: ChannelMomentSpliceRequest;
  readonly revision: number;
  readonly immediate: boolean;
  readonly validationMessage: string | null;
}) {
  const { request, revision, immediate, validationMessage } = input;
  const [response, setResponse] = useState<ChannelMomentSplicePreviewResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const execute = async () => {
      setFailure(null);
      try {
        const result = await previewChannelMomentSplice(request, controller.signal);
        ...
        const error = caught instanceof EvaluationTransportError ? caught : new EvaluationTransportError("RESPONSE", null, "Unexpected Channel moment-splice preview failure.", caught);
